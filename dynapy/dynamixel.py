from dynapy.dynamixel_models import _DynamixelModel
from dynapy.ctrl_table_value import _CtrlTableValue
...
class Dynamixel():
    ...
    def reboot(self):
        """
        Sends a reboot command to the Dynamixel servo. This can be used to restart the servo.
        This method locks the shared U2D2 port during the operation to ensure thread safety.
        """
        with self._u2d2._lock:
            # Send the reboot command to the specific Dynamixel
            result, error = self._u2d2.packet_handler.reboot(self._u2d2.port_handler, self._id)
            if result != COMM_SUCCESS:
                # If the communication was not successful, raise an exception with the error
                error_msg = self._u2d2.packet_handler.getTxRxResult(result)
                raise Exception(f"Failed to reboot Dynamixel ID {self._id}: {error_msg}")
            elif error != 0:
                # If there was a specific error related to the reboot, raise an exception
                error_msg = self._u2d2.packet_handler.getRxPacketError(error)
                raise Exception(f"Error occurred during reboot of Dynamixel ID {self._id}: {error_msg}")
            print(f"Dynamixel ID {self._id} rebooted successfully.")           
